//===-- pPIMTargetMachine.cpp - Define TargetMachine for pPIM ---------------===//
//
...
//===----------------------------------------------------------------------===//
//
// Implements the info about pPIM target spec.
//
//===----------------------------------------------------------------------===//
...
pPIMTargetMachine::pPIMTargetMachine(const Target &T, const Triple &TT,
...
namespace {
class pPIMPassConfig : public TargetPassConfig {
public:
  pPIMPassConfig(pPIMTargetMachine *TM, PassManagerBase &PM)
      : TargetPassConfig(TM, PM) {}

  pPIMTargetMachine &getpPIMTargetMachine() const {
    return getTM<pPIMTargetMachine>();
  }
...
TargetPassConfig *pPIMTargetMachine::createPassConfig(PassManagerBase &PM) {
  return new pPIMPassConfig(this, PM);
}

bool pPIMPassConfig::addInstSelector() {
  addPass(createpPIMISelDag(getpPIMTargetMachine(), getOptLevel()));
  return false;
}

// Force static initialization.
extern "C" void LLVMInitializepPIMTarget() {
  RegisterTargetMachine<pPIMTargetMachine> X(getThepPIMTarget());
}
